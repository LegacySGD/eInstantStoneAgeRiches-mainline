...
function getOutcomeData(scenario, index) {
	var outcomeDatas = scenario.split("|");
	var result = [];
	for (var idx = 0; idx < outcomeDatas.length; idx++) {
		var outcomePairs = outcomeDatas[idx].split(",");
		result.push(outcomePairs[index]);
	}
	return result;
}
function translateOutcomeNumber(outcomeNum, translations) {
	if (outcomeNum.indexOf("X") >= 0) {
		return getTranslationByName("instantWin", translations) + ",  " + outcomeNum;
	} else {
		return outcomeNum;
	}
}
function checkMatch(symbols) {
	...
	var map = {};
	for (var idx = 0; idx < symbolsArr.length; idx++) {
		var key = symbolsArr[idx];
		if (map[key]) {
			var count = map[key];
			count++;
			map[key] = count;
